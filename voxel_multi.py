...
class Feat_Encoder_Decoder(nn.Module):
  def __init__(self, class_num, input_dimensions, out_dim_2D):
    '''
    SSCNet architecture
    ...
    in_channels = input_dimensions    # 64*32  
    f = 32
    
    self.conv_input = nn.Sequential(
      nn.Conv2d(in_channels, 512, kernel_size=3, padding=1, stride=1),
      nn.ReLU(),
      nn.Conv2d(512, 128, kernel_size=3, padding=1, stride=1),
      nn.ReLU(),
      nn.Conv2d(128, f, kernel_size=3, padding=1, stride=1),
      nn.ReLU()
    )
    ...
    self.conv1_1            = nn.Conv2d(int(f/8) + int(f/4) + int(f/2) + int(f), out_dim_2D, kernel_size=3, padding=1, stride=1)
    self.seg_head_1_1       = SegmentationHead(1, 8, self.nbr_classes, [1, 2, 3])

  def forward(self, x):
    input = self.conv_input(x)      
    ...
    scores = out_scale_1_1__3D.permute(0, 1, 3, 4, 2)  # [bs, C, H, W, D] -> [bs, C, W, H, D]
