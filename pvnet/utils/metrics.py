...
class ChamferDistance():
    ...
    def update(self, gt_pcd, pt_pcd):
        ...
        self.dists.append(np.mean(dist_gt_2_pt))
...
class Group(nn.Module):
    ...
    def forward(self, xyz):
        '''
            ...
            center : B G 3
        '''
        center = pointops.fps(xyz, self.num_group)              # 采用FPS采样得到分组的中心点, (1,64,3)
        idx = pointops.knn(center, xyz, self.group_size)[0]     # 找到每个中心点最近的32个目标点的索引
        neighborhood = pointops.index_points(xyz, idx)          # 得到每组距中心点最近的目标点
        neighborhood = neighborhood - center.unsqueeze(2)       # (1,64,32,3)
...
class P2C(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.num_group = 64     # config.num_group
        self.group_size = 32    # config.group_size
        self.mask_ratio = [20, 44]   # config.mask_ratio
        self.nbr_ratio = 2.0    # config.nbr_ratio
        
        self.group_divider = Group(num_group = self.num_group, group_size = self.group_size)
        self.shape_criterion = ChamferDistanceL1()
        self.total_criterion = ChamferDistanceL2()
    ...
    def get_metrics(self, pred, pts):
        total_CD = self.total_criterion(pred, pts)
        ...
        total_points = nbrs + center.unsqueeze(-2)          
        idx = pointops.knn(center, pred, int(self.nbr_ratio * self.group_size))[0]  
        nbrs_pred = pointops.index_points(pred, idx).reshape(B, -1, 3)  
        total_RCD = self.shape_criterion(total_points.reshape(B, -1, 3), nbrs_pred).mean()
        ...
        nbrs_pred = pointops.index_points(pred, idx).reshape(B, -1, 3)
        shape_recon_CD = self.shape_criterion(rebuild_points.reshape(B, -1, 3), nbrs_pred).mean()
        ...
        nbrs_pred = pointops.index_points(pred, idx).reshape(B, -1, 3)
        shape_matching_CD = self.shape_criterion(rebuild_points.reshape(B, -1, 3), nbrs_pred).mean()
        
        return total_CD, total_RCD, shape_recon_CD, shape_matching_CD

    def update(self, CD, RCD, recon_RCD, match_RCD):
        self.avg_CD.append(CD)
        self.avg_RCD.append(RCD)
        self.avg_recon_RCD.append(recon_RCD)
        self.avg_match_RCD.append(match_RCD)
    ...
    def compute(self):
        ...
        length = len(self.avg_RCD)
        for i in range(length):
            avg_CD += self.avg_CD[i]
            avg_RCD += self.avg_RCD[i]
            avg_recon_RCD += self.avg_recon_RCD[i]
            avg_match_RCD += self.avg_match_RCD[i]

        avg_CD /= length
        avg_RCD /= length
        avg_recon_RCD /= length
        avg_match_RCD /= length

        return avg_CD, avg_RCD, avg_recon_RCD, avg_match_RCD
