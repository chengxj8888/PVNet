...
def feats_to_coord(p_feats, resolution, mean, std):
    p_feats = p_feats.reshape(mean.shape[0], -1, 3)     # (b, npoints, 3)
    p_coord = torch.round(p_feats / resolution)         # (b, npoints, 3)
    
    return p_coord.reshape(-1,3)        # (b*npoints, 3)
...
class SparseSegmentCollation:
    ...
    def __call__(self, data):
        ...
        return {'pcd_full': torch.stack(batch[0]).float(),
                'pcd_part' if self.mode == 'diffusion' else 'pcd_noise': torch.stack(batch[1]).float(),
                'filename': batch[2],
        }
